...
export default function ServerAssets() {
  ...
  return <main className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
      ...
      {selectedServerId !== null && <TechCard label="SERVER DETAIL"><div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-semibold text-white">服务器详情</h2><p className="mt-1 text-sm text-slate-400">详情数据来自真实服务器资产记录；探活只通过受控节点回报。</p></div><Button variant="ghost" onClick={() => setSelectedServerId(null)}>关闭</Button></div>{serverDetail.data ? <div className="mt-5 grid gap-4 sm:grid-cols-2"><div><p className="text-xs text-slate-500">主机</p><p className="mt-1 text-sm text-white">{serverDetail.data.hostname}{serverDetail.data.ipAddress ? ` · ${serverDetail.data.ipAddress}` : ""}</p></div><div><p className="text-xs text-slate-500">状态</p><div className="mt-1"><StatusPill status={serverDetail.data.status} /></div></div><div><p className="text-xs text-slate-500">操作系统 / 区域</p><p className="mt-1 text-sm text-slate-300">{serverDetail.data.operatingSystem ?? "—"} · {serverDetail.data.zone ?? "未分区"}</p></div><div><p className="text-xs text-slate-500">负责人 / 凭据</p><p className="mt-1 text-sm text-slate-300">{serverDetail.data.owner ?? "—"} · {serverDetail.data.credentialRef ? "密钥引用" : "未配置"}</p></div><div className="sm:col-span-2"><p className="text-xs text-slate-500">能力标签</p><p className="mt-1 font-mono text-xs text-cyan-100">{serverDetail.data.capabilities?.join(" · ") || "—"}</p></div><div className="sm:col-span-2"><p className="text-xs text-slate-500">探活时间 / 最近消息</p><p className="mt-1 text-sm text-slate-300">{serverDetail.data.lastCheckedAt ? new Date(serverDetail.data.lastCheckedAt).toLocaleString() : "尚无真实探活回报"} · {serverDetail.data.lastProbeMessage ?? "未请求探活"}</p></div><div className="sm:col-span-2"><Button className="bg-cyan-200 text-slate-950 hover:bg-cyan-100" onClick={() => requestProbe(serverDetail.data!.id)} disabled={probeServer.isPending}><Activity className="mr-2 h-4 w-4" />请求受控探活</Button></div></div> : <p className="mt-6 text-sm text-slate-500">正在读取详情…</p>}</TechCard>}
