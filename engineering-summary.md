...
| `pnpm test` | 23 个测试文件、49 个测试全部通过，覆盖受保护回调、初始化登录、弱密码、首次改密门禁、初始化凭据恢复、真实 sessionVersion 失效、服务器治理 SQL 规则、会话轮换、执行状态转换、撤销重试、智能自愈来源及 Runbook→执行编排。 |
| `pnpm build` | 通过，前端静态产物与服务端 Bundle 均成功生成；保留前端主 Bundle 超过 500 kB 的非阻断提示。 |
